...
type WorkshopsPlaceDialogProps = {
...
/**
 * Temporary place for this search (URL `place` only).
 * Saving preferences is explicit via “Bewaar … als mijn regio”.
 */
export function WorkshopsPlaceDialog({
  placeValue,
  placeLabel,
  cities,
  buildHref,
    ...
    <details className="relative">
      ...
      <div className="absolute left-0 z-20 mt-2 w-[min(100vw-2rem,22rem)] rounded-[12px] border border-[var(--border)] bg-[var(--card)] p-4 shadow-[var(--shadow-md)]">
        ...
        <form method="GET" action="/workshops" className="flex flex-col gap-3">
          ...
          <label htmlFor="workshops-place" className="text-sm font-semibold">
            Plaats of stad
          </label>
          <input
            id="workshops-place"
            name="place"
            type="text"
            list="workshops-city-suggestions"
          ...
          />
          <datalist id="workshops-city-suggestions">
            {cities.map((city) => (
        ...
        {savedRegionLabel && !placeValue ? (
          <Link
            href={buildHref({ place: savedRegionLabel, page: undefined })}
